...
export default {
  name: 'GestionTiposServiciosRutas',
  ...
  methods: {
    ...
    async obtenerItems() {
      const response = await this.axios.get('tipos-servicios-rutas', {
        params: {
    ...
    async handleRestore() {
      try {
        await this.axios.post(
          `tipos-servicios-rutas/${this.editedItem.id}/restore`,
        );
    ...
    async handleDelete() {
      try {
        await this.axios.delete(`tipos-servicios-rutas/${this.editedItem.id}`);
