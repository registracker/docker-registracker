    ...
    <v-dialog v-model="restoreDialog" max-width="600">
      <v-card>
        <v-card-title class="text-center">
          ¿Quiere restaurar el registro {{ editedItem.tipo }}?
        </v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="deleteDialog" max-width="600">
      <v-card>
        <v-card-title class="justify-center">
          ¿Quiere eliminar el registro {{ editedItem.tipo }}?
        </v-card-title>
...
export default {
  name: 'GestionTiposVehiculoRutas',
  ...
  methods: {
    ...
    async obtenerItems() {
      const response = await this.axios.get('tipos-vehiculos-rutas', {
        params: {
    ...
    async handleSave() {
      try {
        ...
        await this.axios.post('tipos-vehiculos-rutas', {
          ...this.editedItem,
          id: undefined,
        });
    ...
    async handleUpdate() {
      try {
        ...
        await this.axios.put(`tipos-vehiculos-rutas/${this.editedItem.id}`, {
          ...this.editedItem,
          id: undefined,
        });
    ...
    async handleRestore() {
      try {
        await this.axios.post(
          `tipos-vehiculos-rutas/${this.editedItem.id}/restore`,
        );
    ...
    async handleDelete() {
      try {
        await this.axios.delete(`tipos-vehiculos-rutas/${this.editedItem.id}`);
